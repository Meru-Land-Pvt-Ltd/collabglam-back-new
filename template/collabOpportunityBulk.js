// src/template/collabOpportunityBulk.js
...
const escapeHtml = (s) =>
  ...
  function collabOpportunityBulkTemplate(opts = {}) {
    const subject = "Collab Opportunity";
  
    const name = (opts.influencerName || "").trim() || "there";
    const campaignTitle = (opts.campaignTitle || "").trim() || "our campaign";
    const execName = (opts.executiveName || "").trim() || "Team CollabGlam";
  
    const headline = "Collaboration Opportunity";
  
    const introHtml = `
      Hi <b>${escapeHtml(name)}</b>,<br/><br/>
      We came across your profile and would love to collaborate with you on
      <b>${escapeHtml(campaignTitle)}</b>.<br/><br/>
      If you’re interested, simply reply to this email and we’ll share the next steps.
    `.trim();
  
    const text = `
  Hi ${name},
  
  We came across your profile and would love to collaborate with you on "${campaignTitle}".
  
  If you’re interested, just reply to this email and we’ll share the next steps.
              ...
              <table role="presentation" width="620" cellspacing="0" cellpadding="0"
                ...
                <tr>
                  <td style="padding:22px;font-family:Arial,Helvetica,sans-serif;color:#111827;">
                    <div style="font-size:26px;font-weight:800;margin:0 0 10px 0;">
                      ${escapeHtml(headline)}
                    </div>
                    ...
                    <div style="font-size:13px;color:#111827;margin-top:18px;">
                      Thanks,<br/>
                      <b>${escapeHtml(execName)}</b>
                    </div>
                  </td>
                </tr>
