// src/template/resetOtp.js

/**
 * @param {{ otp: string, role: "Brand"|"Influencer", expiryMinutes?: number, purpose: "signup"|"reset_password" }} opts
 */
function resetOtpEmailTemplate(opts) {
  const expiry = opts.expiryMinutes ?? 10;

  // format OTP like: 7 6 6 5 9 0
  const otpSpaced = String(opts.otp || "")
    .trim()
    .split("")
    .join(" ");

  const isReset = opts.purpose === "reset_password";
  ...
  const subText = isReset
    ? `Use this verification code to reset your password for your <b>${opts.role}</b> account.`
    : `Use this verification code to continue signing up as a <b>${opts.role}</b>.`;

  const text = isReset
    ? `Use this verification code to reset your password for your ${opts.role} account: ${opts.otp}. This code expires in ${expiry} minutes.`
    : `Use this verification code to continue signing up as a ${opts.role}: ${opts.otp}. This code expires in ${expiry} minutes.`;
...
module.exports = { resetOtpEmailTemplate };